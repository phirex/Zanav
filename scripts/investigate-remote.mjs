...
// Use the remote production database
const REMOTE_URL = 'https://nlpsmauwwlnblgwtawbs.supabase.co';
const REMOTE_SERVICE_KEY = process.env.SUPABASE_REMOTE_SERVICE_ROLE_KEY;

console.log('Connecting to remote database:', REMOTE_URL);
console.log('Service key available:', REMOTE_SERVICE_KEY ? 'YES' : 'NO');

if (!REMOTE_SERVICE_KEY) {
  console.error('Missing SUPABASE_REMOTE_SERVICE_ROLE_KEY');
  process.exit(1);
}

const supabase = createClient(REMOTE_URL, REMOTE_SERVICE_KEY);

async function investigate() {
  console.log('🔍 Investigating tenant assignments in REMOTE database...\n');
