...
import {
  bookingRequestCustomerEmail,
  bookingNotificationOwnerEmail,
...
export async function POST(
  ...
  try {
    ...
    const pricePerDay =
      parseFloat(settings.get("default_price_per_day") || "0") || 0;
    ...
    const bookingRows: Database["public"]["Tables"]["Booking"]["Insert"][] =
      insertedDogs!.map((d: any) => ({
        dogId: d.id,
        roomId: room.id,
        ownerId: ownerRow!.id,
    ...
    const { data: created, error: bookErr } = await admin
      .from("Booking")
      .insert(bookingRows)
      .select("id");
    if (bookErr)
    ...
    // Prefer explicit kennel name from settings; fall back to website hero title, then subdomain
    const kennelName =
      settings.get("kennelName") ||
      settings.get("businessName") ||
      (website as any).hero_title ||
      params.subdomain;
    ...
    try {
      if (website.contact_email)
        await sendEmail({
          ...
          subject: `New booking request — ${ownerName}`,
          html: ownerHtml,
        });
      if (ownerEmail)
        await sendEmail({
    ...
    return NextResponse.json({
      success: true,
      bookingIds: created!.map((b: any) => b.id),
    });
  } catch (e: any) {
    console.error(e);
    return NextResponse.json(
